...
class SignalHandler():
    ...
    def create_sockets (self):
        ...
        try:
            ...
            self.log.info("Start controlSubSocket (connect): '{0}'".format(self.controlSubConId))
        except:
        ...
        try:
            ...
            self.log.info("Start comSocket (bind): '{0}'".format(self.comConId))
        except:
            self.log.error("Failed to start comSocket (bind): '{0}'".format(self.comConId), exc_info=True)
            raise
        ...
        try:
            ...
            self.log.info("Start requestFwSocket (bind): '{0}'".format(self.requestFwConId))
        except:
            self.log.error("Failed to start requestFwSocket (bind): '{0}'".format(self.requestFwConId), exc_info=True)
            raise
        ...
        try:
            ...
            self.log.info("requestSocket started (bind) for '{0}'".format(self.requestConId))
        except:
            self.log.error("Failed to start requestSocket (bind): '{0}'".format(self.requestConId), exc_info=True)
            raise
...
#                self.log.info("Connection to {0} is already open".format(socketId))
        ...
        if connectionNotFound:
            ...
            for element in tmpRemoveElement:
                ...
                if variList != None:
                    ...
                    self.log.debug("Remove all occurences from {0} from variable request list.".format(socketId))

                for i in range(len(listToCheck)):
                    if element in listToCheck[i]:
                        listToCheck[i].remove(element)
                        self.log.debug("Remove {0} from pemanent request/allowed list.".format(socketId))
...
class RequestPuller():
    def __init__ (self, requestFwConId, logQueue, context = None):
        ...
        self.log.info("[getRequests] requestFwSocket started (connect) for '{0}'".format(requestFwConId))
...
if __name__ == '__main__':
    ...
    logging.info("=== controlPubSocket connect to: '{0}'".format(controlPubConId))
    ...
    logging.info("=== requestFwSocket connected to {0}".format(requestFwConId))
